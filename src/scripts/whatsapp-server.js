...
server.on("request", async (req, res) => {
  ...
  if (req.url === "/api/sync-chat") {
    ...
    try {
      ...
      const expected = process.env.SYNC_API_KEY || "dev-key-123";

      // Para desarrollo, permitir sin API key o con la key por defecto
      if (!apiKeyHeader && process.env.NODE_ENV === "production") {
        res.writeHead(401, {
          ...corsHeaders,
          "Content-Type": "application/json",
        });
...
function startReminderWorker(
  pollMs = Number(process.env.REMINDER_POLL_INTERVAL_MS || 5000)
) {
...
client.on("ready", async () => {
  ...
  try {
    ...
    try {
      ...
      console.warn(
        "Failed to start reminder worker:",
        e && e.message ? e.message : e
      );
    }
  } catch (err) {
  ...
  try {
    ...
    if (chatIdStr) {
      ...
      try {
        ...
        const chatsForClient = chatsRows.map((c) => ({
          ...c,
          id: { _serialized: c.id },
        }));
        io.emit("chats", {
...
async function saveInitialData() {
  try {
    ...
    if (Array.isArray(chats) && chats.length > 0) {
      ...
      if (
        process.env.PREFETCH_MESSAGES &&
        String(process.env.PREFETCH_MESSAGES).toLowerCase() === "true"
      ) {
